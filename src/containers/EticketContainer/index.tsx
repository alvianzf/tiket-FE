        ...
        isFetching ? (
            ...
            <div className="p-6 md:p-12 flex flex-col gap-10 max-w-[1200px] mx-auto min-h-screen">
                <div className="space-y-4">
                    ...
                    <div className="w-20 h-1.5 bg-cta rounded-full"></div>
                </div>

                <div className="glass-card bg-white/85 p-8 md:p-10 shadow-lg relative overflow-hidden border-white/75">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10 border-b border-slate-200/40 pb-8 mb-8">
                        ...
                        {data?.data.status === 'ISSUED' && (
                            <Button dsVariant="cta" className="rounded-ds-sm px-10 h-14 font-bold text-lg shadow-md hover:scale-105 transition-all w-full md:w-auto" onClick={() => openBase64NewTab(data?.data.tiket_pdf ?? '')}>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-12 py-6 bg-white/40 rounded-ds-lg p-8 border border-white/40 mb-10">
                        <div className="bg-white/80 p-6 rounded-ds-md shadow-inner">
                            <LionAir width={120} height={40}/>
                        </div>
                        
                        <div className="flex-1 w-full space-y-12 font-sans font-medium">
                            {data?.data.flightdetail.map((route, index) => (
                                <div key={index} className="relative">
                                    <div className="flex flex-row items-center justify-between">
                                        <div className="flex flex-col items-center">
                                            <p className="text-3xl font-mono font-bold text-dark">{route.depart}</p>
                                        ...
                                        <div className="flex-1 mx-8 relative flex flex-col items-center">
                                            <div className="w-full h-px bg-gradient-to-r from-transparent via-primary to-transparent relative">
                                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-primary w-2 h-2 rounded-full shadow-md"></div>
                                                <div className="absolute right-0 top-1/2 -translate-y-1/2 text-primary font-bold">✈</div>
                                            </div>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase mt-4 bg-slate-100 px-3 py-1 rounded-full">{route.durationDetail}</p>
                                        </div>

                                        <div className="flex flex-col items-center">
                                            <p className="text-3xl font-mono font-bold text-dark">{route.arrival}</p>
                                    ...
                                    <div className="mt-8 grid grid-cols-2 gap-8 text-center md:text-left">
                                        <div className="space-y-1">
                                            <p className="text-xs font-bold text-slate-400 uppercase leading-none">{route.originName}</p>
                                            <p className="text-sm font-semibold text-slate-600">Terminal -</p>
                                        </div>
                                        <div className="space-y-1 text-right md:text-left">
                                            <p className="text-xs font-bold text-slate-400 uppercase leading-none">{route.destinationName}</p>
                                            <p className="text-sm font-semibold text-slate-600">Terminal -</p>
                                        </div>
                                    </div>
                                    
                                    <div className="mt-4 pt-4 border-t border-dashed border-slate-200 flex justify-center">
                                        <p className="text-sm font-mono font-bold text-primary tracking-widest">{route.flightCode}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center gap-4">
                            <h3 className="text-2xl font-extrabold text-dark tracking-tight">Detail Penumpang</h3>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 mt-12 pt-8 border-t border-slate-100">
                        <Button 
                            dsVariant="cta"
                            className="flex-1 h-14 rounded-ds-sm shadow-md"
                            onClick={() => window.print()}
                        >
                            {t('checkout.download_ticket')}
                        </Button>
                        <Button 
                            dsVariant="primary"
                            className="flex-1 h-14 rounded-ds-sm shadow-md"
                ...
                <div className="glass-card bg-slate-900 border-none text-slate-300 p-10 md:p-12 shadow-2xl space-y-12">
                    <div className="space-y-6">
